...
#masterip=`ifconfig eth0 | grep "inet addr:" | awk '{print $2}' | awk -F ":" '{print $2}'`
masterip=172.17.253.187
echo $masterip
docker_mirror_server="$masterip:5000"
...
echo "iput your answer"
if [ $1 == "install_registry" ];then
    echo "install registry"
    install_registry
fi

if [ $1 == "install_kubetool" ];then
echo "install kubetool"
install_kubetool
fi

if [ $1 == "install_kubecluster" ];then

echo "install_kubecluster"
install_kubecluster

fi


if [ $1 == "set_docker_damon" ];then
   set_docker_damon
fi
